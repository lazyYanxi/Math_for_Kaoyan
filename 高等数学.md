...
## 五、多元函数微分学
1. 多元函数极限
2. 多元函数连续性：$\lim _{(x, y) \rightarrow\left(x_{0} \cdot y_{0}\right)} f(x, y)=f\left(x_{0}, y_{0}\right)$
3. 偏导数
...
4. 全微分
	...
	4. 多元复合函数偏导数求法
	5. 隐函数求导公式
		1. 对于$F(x,y)=0,\frac{\mathrm{d} y}{\mathrm{~d} x}=-\frac{F_{x}}{F_{y}}$
		2. 对于$F(x,y,z)=0,\frac{\partial z}{\partial x}=-\frac{F_{x}}{F_{z}}, \frac{\partial z}{\partial y}=-\frac{F_{y}}{F_{z}}$
		3. 雅可比式：$J=\frac{\partial(F, G)}{\partial(u, v)}=\left|\begin{array}{ll}\frac{\partial F}{\partial u} & \frac{\partial F}{\partial v} \\\frac{\partial G}{\partial u} & \frac{\partial G}{\partial v}\end{array}\right|$

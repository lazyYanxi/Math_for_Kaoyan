1. 单式变式
	...
	7. 凑平方
	8. 三角变式
	9. 等式代换：如题目中给出$f(a)=a$，可以把$a$用$f(a)$代替
	10. 换元
		1. 倒数换元
		2. 三角换元
	11. 积分（等式）：求原函数，逆罗尔处理；分部积分也是一种另类的求导
	12. 求导（等式）
	13. 洛必达（极限）
	14. 泰勒（极限）
	15. 分部积分（积分）
	16. 化为积分：$f(b)-f(a)=\int_a^bf(x)^\prime\mathrm dx$
2. 数列单调增加/减少的两种证明方法
	1. n+1项>n项
	2. n+1项/n项>1
3. 求极限
...
17. 证明题构造方法
	1. 函数构造
		...
		2. 原有式子的某个常数项转化成函数的未知量$x$
			1. 常数有时会隐藏：比如证明$f(1)>1$既可以构造成$F(x)=f(x),x=1$也可以构造成$F(x)=xf(x),x=1$
	2. 定积分构造
		1. 分部积分构造：$\int_a^bf(x)\mathrm dx=\int_a^bf(x)(x-\frac{a+b}{2})^\prime\mathrm dx=\int_a^bf(x)\mathrm d(x-\frac{a+b}{2})$，本质是1的变形
	3. 常用构造思路
		1. 牛顿插值法：$y_{(1)}=y_1+\frac{y_2-y_1}{x_2-x_1}(x-x_1)$，$y_{(2)}=y_1+\frac{y_2-y_1}{x_2-x_1}(x-x_1)+b(x-x_1)(x-x_2)$以此类推，本质是用幂函数拟合未知函数
		2. 梯形近似法：$\int_a^bf(x)\mathrm dx=\frac{f(a)+f(b)}{2}(b-a)-\frac{f^{\prime\prime}(\xi)}{12}(b-a)^{3}$，分部积分构造2次证明
		3. 矩形近似法：$\int_a^bf(x)\mathrm dx=f(\frac{a+b}{2})(b-a)+\frac{f^{\prime\prime}(\xi)}{24}(b-a)^{3}$，2次泰勒中值证明
...
20. 绝对值变式
	...
	2. 平方再根号
	3. 分类讨论
	4. $|a−b|\geqslant|a|−|b|,|a+b|\leqslant|a|+|b|$
21. 三角函数积分特殊公式
...
43. 计算注意事项：开方之后注意正负号
44. 放缩
		1. 对$\sin x$放缩：$x\cos x<\sin x <x$，$\frac{2}{\pi}x<\sin x$
		2. 分式放缩：分子上的放缩是正相关，分母上的放缩是反相关
45. 函数连续性的本质是段上的每一个点都等于邻域的值，也就是$f(a)=\lim_{x\to a}f(a)$
46. 中值定理证明题题目
	1. 点的存在性问题：最简单，大部分是等式运算，本质是利用已知函数性质拟合抽象函数，处理方法也简单，基本变式和中值定理，难点可能在于找到正确的拟合函数（考验变式）
47. 注意定义域！！！
	1. 使用新的未知量要写出定义域
	2. 获得新的式子注意是否有间断点
48. 涉及图形的一定要画图，你才知道是不是你要求的那部分